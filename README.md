...
By default, all tag changes are included. With the `--key`/`-k` argument, only any changes to those tag keys are included in the output. It understands `*` to do prefix matching (e.g. `-k addr:*`), and `*s*` to catch anything that includes the substring `s` (e.g. `-k "*fee*")
